...
class DenseCorrespondenceNetwork(nn.Module):
    ...
    def forward_on_img_tensor(self, img):
        ...
        res = res.data.cuda().numpy().squeeze()
    ...
    def forward_single_image_tensor(self, img_tensor):
        ...
        img_tensor = torch.tensor(img_tensor, device=torch.device("cpu"))
    ...
    def get_unet(config):
        ...
        model = UNet(num_classes=config["descriptor_dimension"]).cpu()
        return model
    ...
    def from_config(config, load_stored_params=True, model_param_file=None):
        ...
        if load_stored_params:
            ...
            #    dcn.fcn.load_state_dict(torch.load(model_param_file, map_location='cpu'))
    ...
    def from_model_folder(model_folder, load_stored_params=True, model_param_file=None,
        ...
        training_config = yaml.safe_load(open(training_config_filename))
    ...
    @staticmethod
    def find_k_best_matches(pixel_a, res_a, res_b, k, debug=False):
        descriptor_at_pixel = res_a[pixel_a[1], pixel_a[0]]
        height, width, _ = res_a.shape
        ...
        #best_match_flattened_idx = np.argmin(norm_diffs)
        best_match_flattened_idx = np.argmin(norm_diffs)
        best_matches_flattened_idxs = np.argpartition(norm_diffs.flatten(), k)[:k]

        best_match_xy = np.unravel_index(best_match_flattened_idx, norm_diffs.shape)
        best_matches_xy = np.unravel_index(best_matches_flattened_idxs, norm_diffs.shape)

        best_matches_diffs = norm_diffs[best_matches_xy]
        best_matches_xy = np.dstack(best_matches_xy).squeeze()
        best_matches_uv = best_matches_xy.copy()
        best_matches_uv[:,[0, 1]] = best_matches_uv[:,[1, 0]]
        
        return best_matches_uv.squeeze().tolist(), best_matches_diffs, norm_diffs
    ...
    @staticmethod
    def find_best_match_for_descriptors_KNN(descriptors, res_a, res_b, k, nbrs=None):
        """
        Compute correspondences for DESCRIPTORS using KNN. This method is 10-20 times
        faster than the brute force method above to query. Training the KNN model takes
        time, so this method is ideal when the model is reused across queries.
        # TOOD(bthananjeyan): finish documentation
        """
        if nbrs is None:
            # if pretrained model is not fed in.
            nbrs = NearestNeighbors(n_neighbors=k, algorithm='kd_tree')
            stacked_descriptors_b = res_b.reshape(-1, res_b.shape[-1])
            nbrs.fit(stacked_descriptors_b)
        query_descriptors = res_a[descriptors[:,1], descriptors[:,0]]
        distances, indices = nbrs.kneighbors(query_descriptors)
        indices = np.unravel_index(indices, res_b.shape[:-1])
        indices = np.stack(indices[::-1]).transpose((1, 2, 0))
        return indices, distances, nbrs
